...
2. **HY-MT 1.5**. Tencent's Hunyuan Translation Model Version 1.5. We use the [1.8B parameter model](https://huggingface.co/tencent/HY-MT1.5-1.8B).
...
The software for this phase can be broken into four stages: (1) building parallel corpora, (2) translating these corpora, (3) evaluating these translations via machine translation metrics, and (4) evaluating these machine translations via human annotation tasks.
Each of these stages corresponds to a module within the `muse` package.

1. `parallel_corpus`: module for building the parallel text corpora used to assess select machine translation models
2. `translation`: module for generating machine translations with select machine translation models
...
- `notebooks/mt_browser.py`: `marimo` notebook for viewing and exploring machine translation corpora and their quantitative evaluation scores
