...
Phase 1 uses two corpora: a sentence-level corpus built from Notion annotation data and a paragraph-level corpus built from side-by-side translations for 7 Music Theory Online (MTO) articles.
...
In Phase 1, the languages of the original text are limited to Chinese (zh), Japanese (ja), Portuguese (pt), and Spanish (es).

### Optional Fields

Sentence- and paragraph-level corpora also have additional optional fields to store corpus-specific information.

#### Parallel Sentence Corpus Fields

A parallel sentence corpus includes additional information extracted from the Notion annotations.

| Field Name | Type | Description |
| --- | --- | --- |
| cite | str | Citation for the original sentence |
| en_cite | str | Citation for the translated sentence |
| term | str | The musical-theoretical concept associated with the sentence |

#### Parallel Paragraph Corpus Fields

A parallel paragraph corpus includes additional citation information for the associated MTO articles.
...
A machine translation corpus is a JSONL file whose records correspond to individual translations by a specific translation model. For Phase 1, a machine translation corpus contains (1) an English translation of each original text within a parallel text corpus and (2) a back translation of each professional English translation within a parallel text corpus.
...
So, the records for an English translation and back translation of a parallel text record will include the following values:
