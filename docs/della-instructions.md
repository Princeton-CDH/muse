...
All HuggingFace models are loaded with `device_map="auto"`, so they use the GPU automatically when `--gres=gpu:1` is present in the Slurm script. No additional configuration is needed.
